...
export default function BusinessForm() {
    ...
    const checkExistingData = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          // Check if email and phone were previously verified
  ...
  const verifyEmail = async () => {
    ...
    try {
  // Only trigger verification if the new email is different from the current one
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await setDoc(businessRef, submittedData, { merge: true });

      // Mark form as filled in users collection
